...
public class ProjectsAction extends BaseAction {
	 ...
	 * 查询工程
	 */
	public String findPageProjects() {
		try {
			Page page = new Page();
			page.setStart(this.getStart());
			page.setLimit(this.getLimit());
			projectsService.findPageProjects(page, title, typeid); // 这里需要修改，需要加入动态获取的用户输入参数；
	...
	public String deleteProjects() {
		try {
			...
			boolean b = projectsService.deleteProjects(contentid);
	 ...
	 * 保存和更新工程
	 */
	public String saveOrUpdateProjects() {
		try {
			// boolean 类型转换处：
			...
			ProjectsDTO dto = new ProjectsDTO(contentid, typeid, typename,
					title, operatetime, displlay, smallpicaddress,
					smallpicname, content);

			projectsService.saveOrUpdateProjects(dto);
			if (contentid == null) {
	...
	public String findTypeType() {
		try {
			...
			this.outListString(projectsService.findTypeType());
		} catch (Exception e) {
			e.printStackTrace();
			this.outError();
		}
