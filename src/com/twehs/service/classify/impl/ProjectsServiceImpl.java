...
public class ProjectsServiceImpl implements ProjectsService {
	...
	public void findPageProjects(Page page, String title, Integer typeid)
		...
		if (typeid != null || (title != null && title.length() != 0)) // 约束显示的情况
		{
			...
			int total = projectsDAO.countAll();
		...
		} else// 全部新闻显示的情况；
		{
			...
			int total = 0;
			List<Type> typeList = typeDAO.listAll(40); // 显示新闻的所有分类
			...
			for (Type type : typeList) {
				List<Content> newslist = projectsDAO.selectByPaper(
						...
						type.getTypeid());
				total = total + projectsDAO.countByTypeid(type.getTypeid()); // 求取pagesize
				if (newslist.size() != 0) {
			...
			@SuppressWarnings("rawtypes")
			List<ProjectsDTO> dtoList = ProjectsDTO.createDtos(dtoListAll);
			page.setTotal(total);
			page.setRoot(dtoList);
		}
	}
